...
void update_force(const Param& param, const Variables& var, array_t& force, elem_cache& tmp_result)
{
    ...
    tensor_t *var_emt_iso_stress = var.emt_iso_stress;
    ...
    #pragma omp parallel for default(none)      \
        shared(var,param,tmp_result,var_connectivity,var_shpdx,var_shpdy,var_shpdz,var_stress,var_emt_iso_stress, \
    ...
    for (int e=0;e<var_nelem;e++) {
        ...
        double *s_iso = (*var_emt_iso_stress)[e];
...
void emt_normal_update_2d(const Variables &var, tensor_t &emt_normal_array, double dt, double w2)
{
        ...
        for (int e=0; e<var.nelem; ++e) {
            ...
            //std::cout << n_new[0] << std::endl;
            //std::cout << "a_n[1]: " << n_new[1] << std::endl;
            //std::cout << n_new[1] << std::endl;
...
void rotate_stress(const Variables &var, tensor_t &stress, tensor_t &emt_iso_stress, tensor_t &strain)
{
    ...
    #pragma omp parallel for default(none) \
        shared(var, stress, emt_iso_stress, strain)
    for (int e=0; e<var.nelem; ++e) {
        const int *conn = (*var.connectivity)[e];
        ...
        jaumann_rate_3d(emt_iso_stress[e], var.dt, w3, w4, w5);
        ...
        jaumann_rate_2d(emt_iso_stress[e], var.dt, w2);
